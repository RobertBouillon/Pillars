...
namespace Spin.Pillars.FileSystem.InMemory
{
  internal class MemoryDirectory
  {
    ...
    public FilePath Path => Parent is null ? new FilePath(EnumerableEx.Single(Name)) : new FilePath(Parent.Traverse(x => x.Parent).Where(x => x.Parent is not null).Reverse().Select(x => x.Name).Concat(Name).ToArray());
